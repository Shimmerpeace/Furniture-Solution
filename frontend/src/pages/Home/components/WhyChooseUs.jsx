...
<h2>Why Choose Us</h2>
<p>
At DecoCreatingShop, we do not just craft new furniture, we craft experiences. Every design reflects sophistication, quality, and comfort.
</p>
</header>
<div className="why-grid">
{items.map((item) => (
<article key={item.id} className="why-card">
<span className="why-number">{String(item.id).padStart(2, "0")}</span>
<h3>{item.title}</h3>
<p>{item.text}</p>
</article>
))}
